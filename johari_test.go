	...
	Describe("testing the johari handlers", func() {
		Context("when internal auth is enabled", func() {
			...
			It("creating and sharing window works", func() {
				...
				By("the new user can acess the feedback page")
				req, err = http.NewRequest("GET", baseURL+"/feedback?pane="+user1Pane, nil)
				req.Header.Add("X-Forwarded-Proto", "https")
				req.AddCookie(user2Cookie)
				resp, err = hc.Do(req)
				...
				By("the new user can acess user info api")
				req, err = http.NewRequest("GET", baseURL+"/get?user=t&pane="+user1Pane, nil)
				req.Header.Add("X-Forwarded-Proto", "https")
				req.AddCookie(user2Cookie)
				resp, err = hc.Do(req)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(resp.StatusCode).To(Equal(200))
				uInfoResp := new(UserInfoResp)
				decoder = json.NewDecoder(resp.Body)
				Expect(decoder.Decode(&uInfoResp)).ShouldNot(HaveOccurred())
				Expect(uInfoResp.Email).To(Equal("testuser1@user.email"))
